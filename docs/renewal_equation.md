...
To find the exact relationship between $r(t)$, $R(t)$, $i(t)$, and $w(\tau)$, differentiate the renewal equation, then divide by $i(t)$:
...
At each time $t$, assume that, over a past time period $a$, $r(t)$ is approximately constant, so that:
...
(Strictly speaking, we should look at the *truncated* moment generating function, but this is only relevant as a transient at the start of the time series.)

For example, if $w(\tau)$ is a Dirac delta function, with mass at some $\tau^\star$, $M_w(-r) = e^{-r \tau^\star}$, so that $R(t) \approx e^{r(t)\tau^\star}$.
If $w(\tau)$ is exponentially distributed, with mean $\tau^\star$, then $M_w(-r) = 1/\left[ 1 + (r/\tau^\star) \right]$ and $R(t) \approx 1 + r \tau^\star$.
